...
On the appliance, `/usr/bin/dadi` is a Nas-built Go CLI that talks to Dimaag’s grantable tool registry. It requires `DIMAAG_URL` (login shells export `http://127.0.0.1:8083` via `/etc/profile.d/dadi-cli.sh`) and exactly one caller identity on execute: `--as-agent-id <uuid>`, `--as-dadi`, or `--as-user`. Flag values that look like JSON arrays or objects are parsed as such. Examples: `dadi help`, `dadi agents`, `dadi nas_get_logs --as-user --level error`, `dadi dimaag_spawn_agent --as-dadi --name "Finance Specialist" --system_prompt "…"`, `dadi ghar_control_device --as-agent-id <uuid> --device_id <uuid> --capability switchable --params '{"state":"on"}'`. Tab completion lists live tools, agent UUIDs for `--as-agent-id`, and `--` parameters. When a stale `~/.local/bin/dadi` would shadow the appliance binary, profile/bashrc put `/usr/bin` ahead on `PATH`.
